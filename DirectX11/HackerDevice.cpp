...
std::unordered_map<uint32_t, uint32_t> vertex_limit_increase = {};
// This will look up a HackerDevice corresponding to some unknown device object
	...
	__out_opt  ID3D11Buffer **ppBuffer)
{
	...
	if (genshin_character_vb_draw_hashes.find(hash) != genshin_character_vb_draw_hashes.end() || vertex_limit_increase.find(hash) != vertex_limit_increase.end()) {

		//LogInfo("Before: %d\n", pNewDesc->ByteWidth);
		UINT new_bytewidth = 8800000;
		auto it = vertex_limit_increase.find(hash);

		if (it != vertex_limit_increase.end()) {
			// Key found, access the associated value
			new_bytewidth = it->second;
		}
